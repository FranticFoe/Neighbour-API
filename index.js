    ...
    try {
        ...
        if (checkLeader.rows.length === 0) {
            const unreadCount = await client.query(
                "SELECT COALESCE(SUM((NOT sender_has_read)::int),0) AS unread_count FROM community_join_request WHERE sender_name = $1",
                [username]
            );
...
app.put("/neighbour/sent_request/has_unread", async (req, res) => {
    const client = await pool.connect();
    const { community_name, username } = req.body
    try {
        await client.query(
...
app.put("/neighbour/join_request/has_unread", async (req, res) => {
    const client = await pool.connect();
